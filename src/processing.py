    ...
    try:
        ...
        if list_dictionary != [] :
            for list_entry in list_dictionary :
                ...
                if not (is_data[:4].isdigit() and is_data[4] == '-' and is_data[7] == '-'
                        and is_data[5:7].isdigit() and is_data[8:10].isdigit()) :
                    return 'Некоректные данные'
                if not (0 < int(is_data[5:7]) < 13 and 0 < int(is_data[8:10]) < 32) :
                    return 'Некоректные данные'
            list_dictionary = sorted(list_dictionary,
                                     key=lambda list_dictionary: list_dictionary["date"], reverse=sort_revers)
        else:
            return 'Некоректные данные'
    except Exception:
        return 'Некоректные данные'
