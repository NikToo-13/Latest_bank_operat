...
def get_mask_account(account: int) -> str:
    ...
    try:
        ...
        logger.info(f"Принимаем номер счета: {account1}")
        if len(account1) == 20:
            account_mask = "**" + account1[-4:]
            logger.info(f"Создаем маску для номера счета: {account_mask}")
        else:
            raise Exception("Проверти, правильность ввода счета!")
        return account_mask
    except Exception as er:
        logger.warning(f"Ошибка: {er}")
        return str(er)
