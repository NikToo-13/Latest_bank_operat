...
from src.utils import convert_date_to_general, json_to_dictionary, transaction_search
...
def main():
    ...
    while True:
        ...
        if status == "EXECUTED" or status == "CANCELED" or status == "PENDING":
            ...
            if isinstance(data_file_filter, list):
                print(f'Операции отфильтрованы по статусу "{status}"')
                break
            elif isinstance(data_file_filter, str):
    ...
    if date_search == []:
        ...
        for date_sample in date_search:
            ...
            if date_sample["operationAmount"]["currency"]["code"] == "RUB":
                ...
                print(
                    f"Сумма: {date_sample['operationAmount']['amount']} "
                    f"{date_sample['operationAmount']['currency']['code']}"
                )
    logger.info(f"{legend_s}Вывод результатов")


if __name__ == "__main__":
    legend_s = "Основная логика "
    logger.info(f"{legend_s}старт")
    ...
    main()
    print("\n**** ЗАВЕРШЕНО ****")
    logger.info(f"{legend_s}- завершено")
